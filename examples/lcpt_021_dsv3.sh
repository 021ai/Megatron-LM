...
TRAIN_TOKENS="$(($(python3 sum_row1.py ${DATASET_FILE})))"
...
# NUM_EXPERTS=120
ROUTER_TOPK=${ROUTER_TOPK:-7}
NUM_SHARED_EXPERTS=${NUM_SHARED_EXPERTS:-1}
MOE_LAYER_FREQ=1
MOE_FIRST_K_DENSE_REPLACE=${MOE_FIRST_K_DENSE_REPLACE:-1}
RMS_NORM_EPS=1e-6

DSA_INDEXER_N_HEADS=${DSA_INDEXER_N_HEADS:-16}  # NUM_ATTN_HEADS/2
DSA_INDEXER_HEAD_DIM=${DSA_INDEXER_HEAD_DIM:-64}  # QK_ROPE_HEAD_DIM
DSA_INDEXER_TOPK=${DSA_INDEXER_TOPK:-256}
DSA_INDEXER_LOSS_COEFF=${DSA_INDEXER_LOSS_COEFF:-0.001}
...
DSA_INDEXER_N_HEADS=${DSA_INDEXER_N_HEADS:-${NUM_ATTN_HEADS}}
DSA_INDEXER_HEAD_DIM=${DSA_INDEXER_HEAD_DIM:-${QK_ROPE_HEAD_DIM}}
DSA_INDEXER_TOPK=${DSA_INDEXER_TOPK:-128}
DSA_INDEXER_LOSS_COEFF=${DSA_INDEXER_LOSS_COEFF:-0.0}
...
MOE_FIRST_K_DENSE_REPLACE=1
RMS_NORM_EPS=1e-6
ROUTER_TOPK_SCALING_FACTOR=2.827
...
dsa_options=""
if [[ ${DSA_DENSE_WARMUP_STAGE:-false} = true ]]; then
    dsa_options=" \
        --dsa-indexer-n-heads ${DSA_INDEXER_N_HEADS} \
        --dsa-indexer-head-dim ${DSA_INDEXER_HEAD_DIM} \
        --dsa-indexer-topk ${DSA_INDEXER_TOPK} \
        --dsa-indexer-loss-coeff ${DSA_INDEXER_LOSS_COEFF} \
        --no-rope-fusion \
        --dsa-dense-warmup-stage \
        "
fi
if [[ ${DSA_SPARSE_TRAINING:-false} = true ]]; then
    dsa_options=" \
        --dsa-indexer-n-heads ${DSA_INDEXER_N_HEADS} \
        --dsa-indexer-head-dim ${DSA_INDEXER_HEAD_DIM} \
        --dsa-indexer-topk ${DSA_INDEXER_TOPK} \
...
if [ $DISPATCHER_TYPE = alltoall_seq ]; then
    ...
    moe_options=" ${moe_options} --moe-token-dispatcher-type flex --moe-flex-dispatcher-backend deepep " # --moe-shared-expert-overlap "
    if [ ${MOE_SHARED_EXPERT_OVERLAP:-false} = true ]; then
        moe_options=" ${moe_options} --moe-shared-expert-overlap "
    fi
elif [ $DISPATCHER_TYPE = flex_hybridep ]; then
    moe_options=" ${moe_options} --moe-token-dispatcher-type flex --moe-flex-dispatcher-backend hybridep " # --moe-shared-expert-overlap "
    if [ ${MOE_SHARED_EXPERT_OVERLAP:-false} = true ]; then
        moe_options=" ${moe_options} --moe-shared-expert-overlap "
    fi
else
...
if [ $PR = fp16 ]; then
   ...
   pr_options=" \
    ...
    if [ ${FP8_DATAFLOW:-false} = true ]; then
        pr_options=" ${pr_options} --moe-expert-input-fp8 --moe-deepep-dispatch-fp8 --moe-expert-fc2-input-fp8"
    fi
    if [ ${FP8_STORE_ACT:-false} = true ]; then
        pr_options=" ${pr_options} --activation-func-fp8-input-store "
    fi
   ...
   pr_options=" \
    ...
    export FP8_GEMM_WGRAD_TRANSPOSE=${FP8_GEMM_WGRAD_TRANSPOSE:-false}
...
if [[ ${MOE_CHUNK:-off} = on ]] ; then
    megatron_options=" ${megatron_options} \
        --moe-chunk-delay-wgrad-compute \
        "
fi
...
if [[ ${PROFILE:-off} = on ]]; then
    ...
    prof_options=" --profile --use-pytorch-profiler --profile-step-start 1 --profile-step-end 2 --profile-ranks 0"
...
if [[ ${USE_TRUNCATELESS_PACKING:-false} = true ]]; then
    megatron_options=" ${megatron_options} \
        --use-truncateless-packing \
        --calculate-per-token-loss
    "
fi
...
if [[ ${MOE_PATHGUARD_LOSS_COEFF:-0.0} != 0.0 ]]; then
    moe_options=" ${moe_options} \
        --moe-pathguard-loss-coeff ${MOE_PATHGUARD_LOSS_COEFF:-0.0} \
        --moe-pathguard-k ${MOE_PATHGUARD_K:-${ROUTER_TOPK}} \
        --moe-pathguard-loss-iter-threshold ${MOE_PATHGUARD_LOSS_ITER_THRESHOLD:-0} \
    "
fi

if [[ ${CONVERT_CKPT:-false} = true ]]; then
    ckpt_options="${ckpt_options} \
        --ckpt-convert-format ${CKPT_CONVERT_FORMAT} \
        --ckpt-convert-save ${CKPT_CONVERT_SAVE_PATH} \
    "
fi

if [[ ${DUMP_PARAM_TO_PARAM_GROUP_MAP_PATH:-none} != none ]]; then
    ckpt_options="${ckpt_options} \
        --dump-param-to-param-group-map ${DUMP_PARAM_TO_PARAM_GROUP_MAP_PATH} \
    "
fi
...
run_cmd="torchrun $DISTRIBUTED_ARGS ${MEGATRON_PATH}/pretrain_gpt.py
 ...
 ${do_options} ${fl_options} ${sp_options} ${moe_options} ${dsa_options} ${offload_option} ${sft_option} ${vp_options} \
 ${uneven_split_option} ${prof_options} ${seqwarm_options} ${new_options} ${fsdp_options} ${ckpt_options} ${pp_layout_option} ${fboverlap_options}"
